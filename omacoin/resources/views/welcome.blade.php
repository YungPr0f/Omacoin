                ...
                <div class="col-lg-6">
                    <div class="call-action-content mt-45">
                        ...
                        <div class="action-btn rounded-buttons">
                            <a href="#" id="sell-now" class="main-btn rounded-two disabled">Sell Now</a>
                        </div>
                    </div>
                </div>
    ...
    $('#sell-now').click(function(e) {
        ...
        @auth
            ...
            @else
                ...
                @if(count($wallets) > 0)
                                                ...
                                                <div class="sign-in-form-wrapper form-style-two receipt-container">
                                                    <div class="form-input hidden">
                                                        ...
                                                        <div class="input-items default">
                                                            <input type="text" name="crypto_amount" placeholder="0.00">
                                                        </div>
                                                    </div>
                                                            ...
                                                            <div class="fileinput fileinput-new w-100 mb-0" data-provides="fileinput">
                                                                ...
                                                                <div class="d-flex justify-content-between light-rounded-buttons danger-buttons">
                                                                    <span class="btn-default btn-file light-rounded-buttons">
                                                                        ...
                                                                        <input name="crypto_receipt" type="file" accept="image/*" />
                                                                    </span>
                    ...
                    sellCrypto.find('.modal').on('show.bs.modal', function() {
                        ...
                        sellCrypto.find('.slick-container').on('beforeChange', function(event, slick, currentSlide, nextSlide) {
                            if (currentSlide == 0 && nextSlide == 1) {
                                ...
                                if(platform == 'Default') {
                                    ...
                                    var walletContents = sellCrypto.find('.wallet-content[data-platform="'+ platform +'"][data-currency="'+ currency +'"]');
                                    ...
                                    walletContents.each(function(index, walletContent) {
                                        ...
                                        if((($(walletContent).attr('data-platform') == platform) || $(walletContent).attr('default') != undefined) && ($(walletContent).attr('data-currency') == currency)) {
                                            ...
                                            if($(walletContent).hasClass('event-added')) { // If wallet has been shown before, do nothing
                                                ...
                                                $(walletContent).find('.qr-visibility').click(function() {
                                                    // var btn = $(this);
                                                    var preview = $(this).parents('.wallet-content').find('.qr-preview');
                                                ...
                                                $(walletContent).addClass('event-added');
                                            }

                                        } else {
                                            $(walletContent).addClass('hidden');
                                        }
                                    });
                                ...
                                // Save Initial Height
                                var initialHeight = $('input:file[name="crypto_receipt"]').parents('.slick-slide').height();
                                
                                // Adjust Height on Select / Change Receipt
                                $('input:file[name="crypto_receipt"]').change(function() {
                                    ...
                                    var observer = new MutationObserver(function( mutations, observer ) {
                                        mutations.forEach(function( mutation ) {
                                            var boxHeight = $(target).parents('.slick-slide').height();
                                ...
                                $('input:radio[name="bank_info"]').change(function() {

                                    var id = $(this).attr('id');
                                    var box = $('.payment-box[data-id="'+ id +'"]');
                                    var boxHeight = box.height();

                                    if(boxHeight < 300) {
                                        $(this).parents('.slick-list').attr('style', 'height: '+ (boxHeight + 26 + 10) +'px');

                                    } else {
                                        //sellCrypto.find('.slick-list').height('auto');
                                        //sellCrypto.find('.slick-slide').height('auto');
                                        //sellCrypto.find('.slick-container').slick('slickSetOption', null, null, true); // Refresh slick
                                        //alert(boxHeight);
                                        
                                        if(box.find('input.nice-select-search').length > 0) {
                                            $(this).parents('.slick-list').attr('style', 'height: '+ boxHeight +'px');

                                        } else {
                                            $(this).parents('.slick-list').attr('style', 'height: '+ (boxHeight + 26) +'px');
                        ...
                        $('form#sell').submit(function(e) {
                            ...
                            if(!($(this).find('input[name="crypto_amount"]').val())) {
                            ...
                            // If existing bank info is selected but there is no form input in the result
                            } else if(($(this).find('input[name="bank_info"]:checked').val() == 'existing') && ($(this).find('.payment-box[data-id="radio_existing"]').find('.form-input').length < 1)) { 
                                toastr.error('Valid bank info is required');
                                
                            // If new bank info is selected AND (any of the inputs are not filled OR the bank dropdown is still default)
                            } else if(($(this).find('input[name="bank_info"]:checked').val() == 'new') && (($(this).find('.payment-box[data-id="radio_new"]').find('input[name]').filter(function() { return $(this).val() == ""; }).length > 0) || ($(this).find('.payment-box[data-id="radio_new"]').find('select').val().startsWith('1')))) {
                                toastr.error('New bank info is incomplete');
                ...
                @else
                    toastr.error('Service currently unavailable');
